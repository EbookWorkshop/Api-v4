...
export class ChapterCollector extends ICollector {
    ...
    async fetch(setting, payload) {
        ...
        if (!isUpdate) {    //检查是否已覆盖更新
            ...
            if (chapt.Content?.length > 10) return this.#sendMessage(payload, false, `章节 ${chapterId} 已有内容，跳过更新。`);
        }
        ...
        do {
            ...
            if (!ctx || !ctx[0].text) { runTime++; console.log("内容采集失败2，已重试：", urlPage); continue; }
            const { text: content } = ctx[0];
            pageCtx.push(content);

            const pg = result.get(RuleName.ContentNextPage);
            if (Error.isError(pg[0])) { runTime++; console.log("内容采集失败3，已重试：", urlPage); continue; }
        ...
        return this.#sendMessage(payload, result, "已完成章节采集");
    }

    #sendMessage(payload, result, message) {
